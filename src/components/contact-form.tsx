...
export function ContactForm() {
  ...
  return <form className="contact-form" onSubmit={submit}>
    <div className="contact-form-head"><span className="eyebrow">Brief de mission</span><h2>Donnez-moi le contexte. Je veux comprendre le problème avant de parler solution.</h2><p>Aucun compte, aucune donnée stockée : le bouton prépare simplement un email dans votre messagerie.</p></div>
    ...
    <div className="form-grid"><label>Vous êtes<select name="profileType" required defaultValue=""><option value="" disabled>Sélectionner</option><option>Entreprise / PME</option><option>Startup / entrepreneur</option><option>Recruteur / ESN</option><option>Association / ESS</option><option>Indépendant</option><option>Autre</option></select></label><label>Organisation<input name="company" autoComplete="organization" /></label></div>
    <div className="form-grid"><label>Type de besoin<select name="requestType" required defaultValue=""><option value="" disabled>Sélectionner</option><option>SaaS / application métier</option><option>CRM sur mesure</option><option>Automatisation / intégrations</option><option>Stripe / monétisation</option><option>Audit / reprise de projet</option><option>Product Owner / pilotage</option><option>Recrutement 100 % remote</option><option>Autre</option></select></label><label>Budget / rémunération<select name="budget" defaultValue=""><option value="">À définir</option><option>Moins de 3 000 €</option><option>3 000 à 10 000 €</option><option>10 000 à 30 000 €</option><option>Plus de 30 000 €</option><option>Poste salarié / TJM à discuter</option></select></label></div>
    <div className="form-grid"><label>Échéance<input name="deadline" placeholder="Ex. septembre 2026" /></label><label>Stack / outils existants<input name="stack" placeholder="Ex. Next.js, Supabase, WordPress…" /></label></div>
