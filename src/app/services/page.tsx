...
export default function Page() {
  return <>
    ...
    />
    <section className="section"><div className="shell grid two">{services.map(s=><article className="card service-card" key={s.title}><div><span className="eyebrow">Expertise</span><h2>{s.title}</h2><p>{s.text}</p></div></article>)}</div></section>
    <section className="section alt"><div className="shell"><SectionTitle eyebrow="Formats" title="Choisir le niveau d’intervention utile." text="Pas de forfait flou : le format dépend du niveau de maturité du produit et du problème à résoudre."/><div className="grid two">{formats.map(([title,text])=><article className="card premium-card" key={title}><span className="eyebrow">Format</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
