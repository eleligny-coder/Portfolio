...
export default function Home() {
  ...
  return <>
    ...
    <section className="section github-proof"><div className="shell github-panel"><div><span className="eyebrow">GitHub & code propriétaire</span><h2>Activité visible. Dépôts applicatifs privés.</h2><p>Les dépôts des produits propriétaires restent privés. Le portfolio expose les fonctionnalités, choix techniques, captures et études de cas nécessaires pour évaluer mon travail sans publier les actifs sensibles.</p><div className="actions"><a className="btn secondary" href={site.github} target="_blank" rel="noreferrer">Voir mon GitHub ↗</a><Link className="btn ghost" href="/cv">Consulter le CV en ligne</Link></div></div><div className="github-terminal" aria-hidden="true"><div><span>●</span><span>●</span><span>●</span></div><code>$ product-builder --status</code><strong>Python / TypeScript / SaaS / CRM / IA</strong><code>$ visibility</code><strong>Contributions publiques + privées</strong><code>$ product-repositories</code><strong>Private / propriétaire</strong></div></div></section>
